    ...
    def process(self, data):
        ...
        if data == self.CHIP_IDLE and not betweenWords:
            ...
            if self.state == "idle":
                ...
                self.logger.info(
                    "Lane {2} Chip event registered with {0} hits, {1} errors".format(
                        self.event.hit_cnt, self.event.errors, self.laneID))
                ...
                self.logger.info(
                    "Lane %d Chip Empty event registered with %d hits, %d errors",
                    self.laneID, self.event.hit_cnt, self.event.errors)
                self.events.append(copy.copy(self.event))
                self.event = None
            elif self.state == "error":
...
class GbtEvent(object):
    ...
    def __str__(self):
        msg = "GBT Event Header Version {0:#X}, Header Size {1}\n".format(self.header_version, self.header_size)
...
class GbtEventDecoder(object):
    ...
    def finalise_event(self, event):
        for channel, decoder in self.decoders.items():
            ...
            decoder.events = []
        # only check if not an SOT, EOT, SOC, EOC event
        #if (event.trigger_type & 0x780) == 0:
        #    bunch_counters = set()
    ...
    def fill_trailer(self, event, trailer):
        ...
        if(values[0] != self.TRAILER_ID):
            self.logger.warning("Expected SOP, but No SOP identifier: {0}".format(values[0]))
            has_error = True
    ...
    def process_event_start(self, readout_board, event):
        ...
        if len(self.results) < 6:
            self.results = self.results + \
                readout_board.read_gbtx_data_from_board(6 - len(self.results))
        if len(self.results) < 6:
    ...
    def process_event_data(self, readout_board, event):
        ...
        while not event_done:
            # load next word
            if not self.results:
                self.results = readout_board.read_gbtx_data_from_board(1)
            ...
            while self.results:
                ...
                if datavalid:
                    ...
                    if channel not in self.decoders:
                        self.decoders[channel] = EventDecoder(channel)
                    payload = data[1:][::-1]
...
def check_event_readout_new(ro_board, nr_events, nr_triggers, lanes,
                            verbose=False, raw_data_file=None):
    gbt_decoder = GbtEventDecoder()
    ...
    lane_event_count = defaultdict(int)

    gbt_events = []
    while total_full_events < nr_triggers and nr_tries < MAX_NR_TRIES:
        ev = gbt_decoder.processEvent(ro_board)
        if verbose:
            logger.info(str(ev))

        gbt_events.append(ev)
        if ev.packet_status['packet_done']:
            total_full_events += 1

        total_errors += ev.error_words
        for lane, sensor_evs in ev.events.items():
            total_chip_events += len(sensor_evs)
            lane_event_count[lane] += len(sensor_evs)
            #if len(sensor_evs) > 1:
            #    logger.warning("More than 1 Event registered at Lane %d", lane)
            total_errors += sum([ev.errors for ev in sensor_evs])
    if verbose:
        logger.info("Lane Event Counts:")
        for l in sorted(lane_event_count):
            logger.info("  Lane %d: %d Events received", l, lane_event_count[l])

    return total_chip_events, total_errors
...
def check_event_readout(cru, nr_events, nr_triggers, lanes,
    ...
    MAX_NR_TRIES = 100
    nr_tries = 0
    decoders = {i: EventDecoder(i) for i in lanes}
