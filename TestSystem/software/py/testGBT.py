...
tb.logger.info("Number of RUv0 FEC Errors at beginning of testGBT = {}".format(fec_errors_beginning))
tb.logger.info("Number of GBTx0 FEC Errors at beginning of testGBT = {}".format(gbtx0_fec_errors_beginning))
...
if (st_flags['GbtRxDataErrorseenFlag'] == 0) and (st_flags['GbtRxReadyLostFlag'] == 0):
    ...
    tb.logger.error("GBTx0 error in PRBS loopback test")
...
if errors == 0:
    tb.logger.info("GBTx0 loopback test completed without errors")
else:
    tb.logger.error("GBTx0 loopback test failed")
