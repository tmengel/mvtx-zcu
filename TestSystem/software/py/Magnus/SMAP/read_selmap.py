...
# }
SMAP_C = {'WRITE': 0x84, 'READ': 0x88, 'ABORT': 0x90,
          'RDWR_DONE': 0xA0}  # commands are (0x80 | (instruction in pkg file))
XIL_ADDR = {'CRC': 0, 'FAR': 1, 'FDRI': 2, 'FDRO': 3, 'CMD': 4, 'CTL0': 5, 'MASK': 6, 'STAT': 7, 'LOUT': 8, 'COR0': 9,
            'MFWR': 10, 'CBC': 11, 'IDCODE': 12, 'AXSS': 13, 'COR1': 14, 'WBSTAR': 16, 'TIMER': 17, 'BOOTSTS': 22,
            'CTL1': 24, 'BSPI': 31}
NOOP = 0x20000000
# INIT deletes Xilinx
# RD PAGE : READ
...
class Smap():
    ...
    def CMWP(self, framedata, address):
        """writes a frame"""
        return self.continuous_write(framedata, address)

    def write(self, data):
        self.regs.SMAP_DATA_RX.write(data)
        self.regs.SMAP_CMD.write(SMAP_C['WRITE'])
        # self.pa3.write(SMAP_DATA_RX, data)  # insert data
        # self.pa3.write(SMAP_CMD, SMAP_C['WRITE'])  # GO

    def read(self):
        self.regs.SMAP_CMD.write(SMAP_C['READ'])
        data = self.regs.SMAP_DATA_TX.read()
    ...
    def CMRP(self, startaddr=0, nframes=1, ignorefs=0):
        ...
        for i in range(123*nframes): # not sure if *nframes works.
    ...
    def continuous_write(self, framedata, address, nframes=1):
        CMD = self.type1_w(XIL_ADDR['CMD'])
    ...
    def end(self):
        ...
        self.regs.SMAP_CMD.write(SMAP_C['RDWR_DONE'])

    def abort(self):
        # self.pa3.write(SMAP_CMD, SMAP_C['ABORT'])
        self.regs.SMAP_CMD.write(SMAP_C['ABORT'])
...
if __name__ == "__main__":
    ...
    try:
        ...
        sm = Smap(pa3)
        print("CTL0:")
        print(hex(sm.sm_read(XIL_ADDR['CTL0'])))  # sm.sm_read(XIL_ADDR[rwadr])
        ...
        # print(sm.rd_frames([0]))
        # test_sm_rw(sm)
        # gold_frame,gfadr = test_frame_rw(sm)
        # sm.continuous_write(gold_frame[0],gfadr)
        #test_multiframe_rw(sm)
