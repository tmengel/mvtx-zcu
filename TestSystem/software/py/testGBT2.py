...
tb.logger.info("Number of RUv0 FEC Errors at beginning of testGBT2 = {}".format(fec_errors_beginning))
tb.logger.info("Number of GBTx2 FEC Errors at beginning of testGBT2 = {}".format(gbtx2_fec_errors_beginning))
...
if (st_flags['GbtRxDataErrorseenFlag'] == 0) and (st_flags['GbtRxReadyLostFlag'] == 0):
    ...
    tb.logger.error("GBT2 error in PRBS loopback test")
...
if errors == 0:
    tb.logger.info("GBT2 loopback test completed without errors")
else:
    tb.logger.error("GBT2 loopback test failed")
